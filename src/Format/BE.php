...
class BE implements Vies
{
    ...
    {
        $vatNumber = is_string($vatNumber) ? preg_replace('/^BE|[\-.\s]/i', '', $vatNumber) : '';
    ...
    {
        $int = (int) substr($vatNumber, 0, 8);
        $controlCode = 97 - $int % 97;

        return (int) substr($vatNumber, -2) === $controlCode;
    }
}
