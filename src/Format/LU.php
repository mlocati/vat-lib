...
class LU implements Vies
{
    ...
    {
        $vatNumber = is_string($vatNumber) ? preg_replace('/^LU|[\-.\s]/i', '', $vatNumber) : '';
    ...
    {
        $int = (int) substr($vatNumber, 0, 6);
        $controlCode = (string) $int % 89;

        return substr($vatNumber, -2) === str_pad((string) $controlCode, 2, '0', STR_PAD_LEFT);
    }
}
